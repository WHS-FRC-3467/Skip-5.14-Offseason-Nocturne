...
public class ThriftyNova {
    ...
    /**
     * Drives the motor towards the given position using the configured P, I, D and
     * F values.
     */
    ...
    /**
     * Drives the motor at the given velocity using the configured P, I, D and F
     * values.
     */
    ...
    public boolean setInverted(boolean inverted) {
        return setConfig(0, inverted ? 1 : 0);
    }

    public boolean setBrakeMode(boolean brake_mode) {
        return setConfig(1, brake_mode ? 1 : 0);
    }

    /**
     * Sets the max forward speed of the motor which the motor will not excede, even
     * if instructed to.
    ...
    /**
     * Sets the max reverse speed of the motor which the motor will not excede, even
     * if instructed to.
     * Input ranges from 0 to -1. Will return true on successful upload.
     */
    ...
    /**
     * Sets the ramp rate in seconds. For example, an input of .5 will ramp the
     * motor from idle to 100%
     * over the course of .5 seconds. Input ranges from 0 to 10. Will return true on
     * successful upload.
     */
    public boolean setRampUp(double ramp_up) {
        ...
        if (ramp_up != 0)
            translated_ramp = 1 / (ramp_up * 1000);
    ...
    public boolean setRampDown(double ramp_down) {
        ...
        if (ramp_down != 0)
            translated_ramp = 1 / (ramp_down * 1000);

        return setConfig(6, translateOutput(translated_ramp));
    }

    public boolean setMaxCurrent(double max_current) {
        return setConfig(7, translateOutput(max_current));
    }

    public boolean setExtSensorId(int extern_id) {
        return setConfig(9, extern_id);
    }

    public boolean setkP(double p) {
        return setkP(p, 0);
    }
    ...
    public boolean setSFPeriodFaults(double period) {
        return setConfig(18, translateOutput(period));
    }

    public boolean setSFPeriodSensor(double period) {
        return setConfig(19, translateOutput(period));
    }

    public boolean setSFPeriodExtSensor(double period) {
        return setConfig(20, translateOutput(period));
    }

    public boolean setSFPeriodControl(double period) {
        return setConfig(21, translateOutput(period));
    }

    public boolean setSFPeriodCurrent(double period) {
        return setConfig(22, translateOutput(period));
    }

    /// GETTERS
    public int getPosition() {
        return getMotorStatus(1, 4, 7);
    }

    public int getVelocity() {
        return getMotorStatus(1, 0, 3);
    }

    public int getExtPosition() {
        return getMotorStatus(2, 4, 7);
    }

    public int getExtVelocity() {
        return getMotorStatus(2, 0, 3);
    }

    public int getCurrentDraw() {
        return getMotorStatus(4, 6, 7);
    }
    ...
    private boolean setConfig(int index, int value) {
        if (CANAPIJNI.writeCANPacketNoThrow(
                ...
                getApiId(CONTORL_API_CLASS, 0)) != 0)
            return false;

        CANData data_in = new CANData();
        long rio_time = RobotController.getFPGATime();

        /// Checks if robot disabled because 1. blocking isn't as much of an issue and
        /// 2. rio start up times
